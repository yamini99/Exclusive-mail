    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(session==null){
            ...
            try{
                ...
                PreparedStatement ps=con.prepareStatement("select * from mail_message where (receiver=? OR sender=?) and sendtrash=? order by id desc");
                ...
                ps=con.prepareStatement("select * from mail_message where (receiver=? OR sender=?) and rectrash=? order by id desc");
                ps.setString(1,email);
                ps.setString(2,email);
                ps.setString(3,"yes");
                rs=ps.executeQuery();
